...
class RevSliderOperations extends RevSliderElementsBase{
	...
	<head>
        ...
        <!-- ADD-ONS CSS FILES -->
        <?php foreach ($CI->data->get('styles') as $_handle => $_style) : ?>
            <link  rel="stylesheet" type="text/css"  media="all" href="<?php echo $_style; ?>" />
        <?php endforeach; ?>

        <!-- ADD-ONS JS FILES -->
        <?php foreach ($CI->data->get('scripts') as $_handle => $_script) : ?>
...
ob_end_clean();
		...
		$search = array(
            ...
            'url":"',
            'image":"',
            rtrim($cont_url, RS_IMAGE_PATH) . $addonPath,
            rtrim($cont_url_no_www, RS_IMAGE_PATH) . $addonPath,
